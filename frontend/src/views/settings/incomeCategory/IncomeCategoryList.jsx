...
import { DeleteIncomeCategoryApi, IncomeCategoryListApi } from 'apis/Settings';
import useAuth from 'hooks/useAuth';
import { apiErrorSnackBar, apiSuccessSnackBar, apiValidationSnackBar } from 'utils/SnackBar';
import AddEditIncomeCategory from './AddEditIncomeCategory';
...
export const IncomeCategoryList = ({ search, callApi, setInitData, initData }) => {
    ...
    const deleteHandler = () => {
        DeleteIncomeCategoryApi({ id: categoryData.id })
            .then((res) => {
    ...
    return (
        <>
                            ...
                            data.map((item, idx) => (
                                <TableRow key={idx}>
                                    ...
                                    <TableCell align="right">
                                        {checkRestriction('CAN_EDIT_INCOME_CATEGORY') && (
                                            <IconButton color="primary" component="label" onClick={() => editData(item)}>
                                                <EditIcon fontSize="small" />
                                            </IconButton>
                                        )}
                                        {checkRestriction('CAN_DELETE_INCOME_CATEGORY') && (
            ...
            {openEdit && (
                ...
                >
                    <AddEditIncomeCategory value={categoryData} formID="editIncomeCategory" onSubmit={submitHandler} initData={initData} />
                </CenterDialog>
            )}
            {openDelete && (
                <DeleteDialog
