...
import { ContributionRatioListApi, deleteContributionRatioApi } from 'apis/Settings';
...
import AddEditContributionRatio from './AddEditContributionRatio';
...
const ContributionRatioList = ({ search, callApi }) => {
    ...
    const [contributionRatioData, setContributionRatioData] = useState({});
    ...
    const getData = () => {
        ContributionRatioListApi(params)
            .then((res) => {
    ...
    const deleteHandler = () => {
        deleteContributionRatioApi({ id: contributionRatioData.id })
            .then((res) => {
    ...
    return (
        <>
                    ...
                    <TableBody>
                        {data.map((item, Index) => (
                            <TableRow key={Index}>
                                ...
                                <TableCell align="left">{item.ratio}</TableCell>
            ...
            {openEdit && (
                <CenterDialog
                    title="Edit Contribution Ratio"
                    open={openEdit}
                    onClose={() => setOpenEdit((prevState) => !prevState)}
                    id="editContributionRatio"
                >
                    <AddEditContributionRatio value={contributionRatioData} formID="editContributionRatio" onSubmit={submitHandler} />
                </CenterDialog>
            )}
            {openDelete && (
                <DeleteDialog
                    ...
                    open={openDelete}
                    dept="Contribution Ratio"
                    name={contributionRatioData['title']}
                />
            )}
        </>
    );
...
ContributionRatioList.propTypes = {
    search: propTypes.string,
    callApi: propTypes.bool
};

export default ContributionRatioList;
