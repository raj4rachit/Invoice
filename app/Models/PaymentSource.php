...
class PaymentSource extends Model
{
    ...
    protected $allowedFields    = ['subscriber_id', 'payment_source_name', 'status', 'created_by', 'updated_by', 'created_at', 'updated_at'];
    ...
    public function getResource($filters = array(), $returnAssoc = false, $returnSingleRow = false)
    {
        ...
        $builder->select('payment_sources.*, CONCAT(createdBy.first_name," ", createdBy.last_name) as created_by, CONCAT(updatedBy.first_name," ", updatedBy.last_name) as updated_by');

        $builder->join('users as createdBy', 'createdBy.id = payment_sources.created_by', 'LEFT');
        $builder->join('users as updatedBy', 'updatedBy.id = payment_sources.updated_by', 'LEFT');

        (isset($filters['id']) && $filters['id'] != '') ? $builder->where('payment_sources.id', $filters['id'])
            : $builder->where('payment_sources.subscriber_id', $userData['subscriber_id']);

        if (isset($filters['search']) && $filters['search'] != '') {
            $search = $this->db->escapeString((string) $filters['search']);
            $builder->like('payment_sources.payment_source_name', $search);
        }
